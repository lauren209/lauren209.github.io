...
describe('Tutorial part 5', () => {
  ...
  function getPageElts() {
    ...
    return {
      ...
      appTripsHref: navElts.get(0),
      appTrips: element(by.css('app-root app-trips')),
      topHeroes: element.all(by.css('app-root app-trips > div h4')),
  ...
  describe('Initial page', () => {
    ...
    const expectedViewNames = ['Trips', 'Heroes'];
    it(`has views ${expectedViewNames}`, () => {
      let viewNames = getPageElts().navElts.map((el: ElementFinder) => el.getText());
      expect(viewNames).toEqual(expectedViewNames);
    });

    it('has trips as the active view', () => {
      let page = getPageElts();
      expect(page.appTrips.isPresent()).toBeTruthy();
    });
  ...
  describe('Trips tests', () => {
    ...
    it(`selects and routes to ${targetHero.name} details`, tripsSelectTargetHero);

    it(`updates hero name (${newHeroName}) in details view`, updateHeroNameInDetailView);

    it(`saves and shows ${newHeroName} in ?Trips`, () => {
      element(by.buttonText('go back')).click();
      let targetHeroElt = getPageElts().topHeroes.get(targetHero?TripsIndex);
  ...
  async function tripsSelectTargetHero() {
    let targetHeroElt = getPageElts().topHeroes.get(targetHeroTripsIndex);
